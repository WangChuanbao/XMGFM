...
@interface XMGDownLoader()<NSURLSessionDelegate>
...
- (void)cancelDownLoad {
    ...
    // 清空缓存
    [XMGFileTool removeFileAtPath:self.fileFullPath];
}
...
+ (long long int)cacheFileSizeWithURL:(NSURL *)url {
    ...
    return  [XMGFileTool getFileSizeWithPath:path];
...
+ (void)removeCacheFileWithURL:(NSURL *)url {
    ...
    NSString *path = [kLocalPath stringByAppendingPathComponent:url.lastPathComponent];
    [XMGFileTool removeFileAtPath:path];
...
- (BOOL)checkLocalFile {
    ...
    self.fileCurrentSize = [XMGFileTool getFileSizeWithPath:self.fileFullPath];
